...
| 갱신 방법 | 실행 파이프라인 `스마트 실행` | 로컬 계산 후 자동 또는 `Start-KR-Quant-Public.bat` |
...
- 각 메뉴 상단에서 기준일, 출처, 표본/범위, 계산 상태, 퀀트 반영 여부, 한계를 공통 근거 계약으로 확인
- 각 숫자에 공식/관측/모델, 계산식, 누락, 경고를 붙임
...
2. **실행 파이프라인**에서 `오늘 필요한 작업 스마트 실행`을 누릅니다. 시세·재무·퀀트·수급 중 필요한 단계만 실행합니다.
3. 품질검사를 통과하면 공개 웹도 자동 갱신됩니다. 수동 업로드가 필요할 때만 `Start-KR-Quant-Public.bat`을 실행합니다.
