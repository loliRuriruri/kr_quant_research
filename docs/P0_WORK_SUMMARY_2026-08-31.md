...
인계서 기준 **P0는 P0-6까지 끝났다.** **P1-1(공식 기업행위)과 P1-2(시점 유니버스)도 코드에 들어갔다.**
...
1. **P1-2** 진짜 시점 유니버스. 상장폐지일 커버리지 0%라 현재 TOP20 소급 연구를 시장 전체처럼 말하면 안 된다.
2. **P1-3** 스마트 작업 관측성. 장시간 작업이 `[1/4]`에 멈춰 보이는 문제, heartbeat, 취소.
3. **P1-4** 백테스트 민감도·표본 경고. 이미 있는 체결 프록시를 과장하지 않게.
4. **P1-5** 시장 국면 점수 재현. 금리·환율·VIX 각각의 날짜와 기여도.
