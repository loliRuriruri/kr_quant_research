...
- Do not modify `season_jev_shadow.py` / `season_jev_budget.py` in the first slice (import read-only helpers if useful).
...
5. **Quant-forbidden fields in calibration state** (Task 2 + Task 5): recursive rejection of `quantReference`, `quant_reference`, `pre_entry_rank`, `grade`, `seasonality_score`, `score_breakdown`.
...
| Path | Note |
|---|---|
| `src/kr_quant/research/season_jev_shadow.py` | Read-only import of path helpers / `FORBIDDEN_STATE_KEYS` / `assert_state_clean` / `NOUL_HEADS` allowed; no edits |
...
python scripts/jev_calibration_export.py ingest-labels --labels PATH --out PATH
...
Four samples for head `needsNews` (plus one holdout decoy):
...
At threshold `0.50` on calibration-valid rows only:

- TP=1 (c1), FP=1 (c3), TN=0, FN=1 (c2); c4 excluded as unknown
- precision=0.5, recall=0.5, FPR=`None` (FP+TN=0), FNR=0.5, unknown_count=1

Sweep candidate set must be `sorted({0.0, 1.0, 0.80, 0.40, 0.70, 0.90})` and must ignore holdout probability `0.10`.
...
def test_confusion_and_null_safe_rates():
...
def test_sweep_uses_observed_boundaries_and_calibration_only():
    ...
    assert thresholds == sorted({0.0, 1.0, 0.80, 0.40, 0.70, 0.90})
...
```text
C:\Users\a4jud\kr_quant_research\.venv\Scripts\python.exe -m pytest tests/unit/test_jev_calibration.py -q --tb=line -k "confusion or invalid_probability or sweep_uses or fn_sensitive"
```
...
Produces: `build_calibration_report`, `lock_selection`, `selection_manifest_hash`, `evaluate_holdout_locked`, `attach_review_status`, `mark_holdout_non_pristine`, state constants
...
def test_holdout_before_lock_unavailable():
    with pytest.raises(cal.CalibrationError, match="CALIBRATION_OPEN|SELECTION_LOCKED|holdout"):
        cal.evaluate_holdout_locked(
            samples=[], head="needsDart", selected_threshold=0.7,
            selection={"holdout_revealed": False, "state": cal.STATE_CALIBRATION_OPEN},
        )

def test_lock_selection_calibration_only_and_hash():
    manifest = {
        "dataset_id": "d1",
        "dataset_hash": "a" * 64,
        "bucket": {"provider": "typesafe_direct", "requested_model": "jev-latest",
    ...
    with pytest.raises(cal.CalibrationError):
        cal.lock_selection(dict(manifest, selection_basis="holdout"))

def test_holdout_eval_only_locked_threshold_no_alt_winner():
    samples = [
    ...
    selection = cal.lock_selection({
        ...
        "bucket": {"provider": "typesafe_direct", "requested_model": "jev-latest",
                   "evaluator_version": "season-jev-shadow-v1"},
        "head": "needsDart", "selected_threshold": 0.5,
        "selection_basis": "calibration_only", "calibration_metrics_hash": "b" * 64,
        "holdout_revealed": False,
    })
    report = cal.evaluate_holdout_locked(
        samples=samples, head="needsDart", selected_threshold=0.5, selection=selection,
    )
...
- Calibration report may include support counts and `INSUFFICIENT_CLASS_SUPPORT` flags (compute/report only; never auto-enable JEV).

- [ ] **Step 2: RED**

```text
C:\Users\a4jud\kr_quant_research\.venv\Scripts\python.exe -m pytest tests/unit/test_jev_calibration.py -q --tb=line -k "calibration_report or holdout_before or lock_selection or holdout_eval or non_pristine"
```
...
## Task 5: Shadow Export / Label Ingest CLI + Runtime Isolation

**Interfaces:**

Consumes: shadow JSON shape from `season_jev_shadow` artifacts; Task 2–4 functions; `write_json_atomic`

Produces: `calibration_root`, `export_candidates_from_shadow`, CLI `scripts/jev_calibration_export.py`, `.gitignore` entry `data/research/jev_calibration/`

### Steps

- [ ] **Step 1: Failing tests (Review Focus #5 for export path)**
...
def test_export_candidates_from_shadow_strips_quant_and_sets_ids():
    shadow_payload = {
        "generation_id": "g1",
        "results": [{
            "id": "sig-1",
    ...
    assert "quant_reference" not in rows[0]
    cal.assert_calibration_state_clean(rows[0]["state"])

def test_export_rejects_quant_in_state():
    with pytest.raises(ValueError):
        cal.export_candidates_from_shadow(
            {"results": [{"id": "x", "ticker": "1", "state_hash": "h",
...
Also add a CLI help test (subprocess or imported `main`) asserting subcommands:

`export-candidates`, `ingest-labels`, `calibration-report`, `lock-selection`, `holdout-eval`.

Blind-label template rule for `ingest-labels` / export template: human label fields present; JEV probability fields omitted or redacted until after commit metadata `blind=true` workflow documented in CLI `--help` text (no UI).

- [ ] **Step 2: RED**

```text
C:\Users\a4jud\kr_quant_research\.venv\Scripts\python.exe -m pytest tests/unit/test_jev_calibration.py -q --tb=line -k "calibration_root or export_candidates or export_rejects or cli_help"
```

- [ ] **Step 3: Implement** thin CLI calling core; add `.gitignore` line `data/research/jev_calibration/`; persist with `write_json_atomic`; read shadow files read-only; **no live JEV API calls**; do not write into main data lake roots (`data/raw`, `data/staged`, etc.).
...
```text
git add src/kr_quant/research/jev_calibration.py scripts/jev_calibration_export.py tests/unit/test_jev_calibration.py .gitignore
git diff --cached --check
...
- `season_jev_shadow.py` and `season_jev_budget.py` unchanged vs plan base `92be125...` unless a prior approved exception exists
...
- `season_jev_shadow.py` left unchanged in first slice
