...
def test_demo_pipeline_deterministic(settings, tmp_path):
    ...
    # output files
    out = settings.output_dir / "as_of_date=2024-12-30"
    assert (out / "top20.csv").exists()
    assert (settings.output_dir / "latest_top20.csv").exists()
    assert (settings.output_dir / "data_quality_report.json").exists()
    assert (settings.output_dir / "daily_history.parquet").exists()
    assert (settings.output_dir / "latest_all_stocks.parquet").exists()
